...
| `Unsupported codec: numcodecs.shuffle` (or `numcodecs.zlib`, `numcodecs.delta`, …) | A **v2→v3 converted** store declares codecs under the `numcodecs.*` namespace, and GDAL's **v3** codec dispatch is built around core v3 names. Standalone shuffle *is* implemented but only wired into the **v2** path; `numcodecs.zlib` is just an unaliased name for core zlib. | Update GDAL once the v3 dispatch maps the `numcodecs.*` family. Distinct from pcodec (which has no implementation at all) — these are wiring/alias gaps over existing code. Repro: ismip6 `icechunk-ais` `/combined/AWI_PISM1/exp02/ligroundf` (chain `bytes → numcodecs.shuffle(elementsize:4) → numcodecs.zlib(level:3)`). |
