...
| Opens, lists metadata, **but a data read fails** with `Unsupported codec` / `codec not available` (`pcodec`, `numcodecs.shuffle`, `numcodecs.zlib`, `numcodecs.delta`, …) | The store uses a **non-core** Zarr v3 codec, declared via the numcodecs-zarr3 shim. GDAL's v3 driver supports **core codecs only** (`bytes`, `blosc`, `gzip`, `zstd`, `crc32c`, `sharding_indexed`). Non-core ≠ illegitimate: **pcodec is in heavy real-world use** (e.g. Earthmover ERA5). | **Upstream wontfix for GDAL** (non-standard codecs out of scope for now), so metadata reads but values don't. The store is still readable — by a **numcodecs-aware reader**: zarr-python, or a Rust reader with the codec feature (zarrs / your `zr` lineage has pcodec). This is the GDAL-vs-Rust path split in practice. Severities: `numcodecs.shuffle` is non-core but has a v2-path impl (one barrier: scope); `pcodec` is non-core *and* unimplemented in GDAL (two barriers). |
...
| `manifest extents has not expected dimension count` on a `crs`/`spatial_ref` array | **A malformed manifest from a non-conformant writer**, not a rank-handling gap. The scalar array's Zarr `shape`/`node_data.shape[]` are correctly empty, but its `manifests[0].extents` was written as `[{from:0,to:1}]` (a phantom 1-D extent on a 0-D array) instead of empty `extents[]`. **Fixed** in the driver (now tolerant). Seen on the ISMIP6 AIS store, writer header `ic-0.3.16` (no public pip release). | Update GDAL. If you must read an affected store on an old build, the offending var is the scalar grid-mapping container. Stores from that writer may carry other non-conformant details — sanity-check values against Python/zarrita. |
